...
from applications.pieza.views import ListaSubTarea
from applications.home.views import writeAndReadJson, readJson
from applications.pieza.models import filaCarrito, Tarea
...
def seccion(request):
    ...
    """
    la variable "aparato" obtiene desde el archivo aparato.html
    la opción según selecciona el usuario desde la página.
    """
    aparato = request.GET.get("aparato",'')
    ...
    """
    options = {
        'Cabezal 622': 'Cabezal 622',
        '':''
    }

    """
    La siguiente línea añade al archivo historial.json la 
    clave 'aparato' con su valor respectivo options[aparato].
    """
    historial = writeAndReadJson('aparato',options[aparato])
    ...
    """
    Lo siguiente corresponde a la vista de una sección en particular.
    """
    if request.GET.get("seccion",''):
        ...
        """
        la variable "seccion_elegida" obtiene desde el archivo seccion.html
        la opción según selecciona el usuario desde la página.
        """
        seccion_elegida = request.GET.get("seccion",'')


        return redirect(reverse('pieza_app:pieza')+"?pieza=%s" % seccion_elegida)

    

    return render(request,'seccion/seccion.html',context)